...
if(_selection == 0) then{ //backpack
	...
	if (!isNil "_backpack") then {  
		...
		_totalSpace = 10 * _currentPrim + 5 * _currentSec + _currentTool + count(_normalMags) + 10 * _primaryToBuy + 5 * _sidearmToBuy + _toolsToBuy;
		_totalNewSpace = 10 * _primaryToBuy + 5 * _sidearmToBuy + _toolsToBuy;
	}else {
	...
	if( _totalSpace >= _allowedMags)then{
		...
		systemChat format["Total space succeeded: Mag=1, Tool=1, Side=5, Primary=10 slots and you have %1 open slots and tried to put in %2 slots", _allowedMags, _totalNewSpace];
	};
